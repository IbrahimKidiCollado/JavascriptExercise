...
export class GradeSchool {
	...
	roster() {
		...
		if (Object.keys(this.school).length === 0) return [];

		for (let i in this.school) {
			this.school[i].sort();
		}
		Object.keys(this.school).sort((a, b) => a - b);

		return Object.values(this.school).flat();
	}

	add(name, grade) {
		if (this.school.hasOwnProperty(grade)) {
			for (let i in this.school) {
				if (this.school[i].includes(name)) return false
			}
